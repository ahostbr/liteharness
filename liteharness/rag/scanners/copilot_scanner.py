"""CopilotScanner — indexes GitHub Copilot conversation transcripts from ~/.copilot/.
...
def _parse_messages(path: str) -> list[dict]:
    ...
    try:
        ...
        logger.warning(f"copilot_scanner: {path}: {exc}")
    return messages


class CopilotScanner(BaseScanner):
    def scan_paths(self) -> list[str]:
        if not _COPILOT_DIR.exists():
            return []
        paths = []
        for ext in ("*.jsonl", "*.json"):
            for f in _COPILOT_DIR.rglob(ext):
                paths.append(str(f))
        return paths
